...
CUDA_HOST_DEVICE __noinline__ real3 Fast_Eigenvalues(const SymMat33& A)  // 24 mults, 20 adds, 1 atan2, 1 sincos, 2 sqrts
{
...
CUDA_HOST_DEVICE Mat33 __noinline__ Fast_Eigenvectors(const SymMat33& A, real3& lambda) {
...
CUDA_HOST_DEVICE void Fast_Solve_EigenProblem(const SymMat33& A, real3& eigen_values, Mat33& eigen_vectors) {
    eigen_values = Fast_Eigenvalues(A);
    eigen_vectors = Fast_Eigenvectors(A, eigen_values);
}

CUDA_HOST_DEVICE void SVD(const Mat33& A, Mat33& U, real3& singular_values, Mat33& V) {
